...
export default function UnadjustedTrialBalancePage() {
  ...
  // Memuat data Unadjusted Trial Balance dari API Backend Web
  const fetchTrialBalanceData = useCallback(async () => {
    setLoading(true);
    setErrorMessage(null);
    try {
      const response = await apiClient.get(`${API_BASE_URL}/api/v1/reports/trial-balance/unadjusted`, {
        method: 'GET',
        headers: {
      ...
      if (response.status !== 200 && response.status !== 201) {
        throw new Error('Failed to load Unadjusted Trial Balance data from the server.');
      }
      ...
      console.error('Error fetching unadjusted trial balance:', error);
      ...
      {noPeriodSelected ? (
        <div className="text-center py-5 my-4">
          ...
          <p className="text-white-50 mb-3">
            This report follows whichever period you&apos;re viewing. Select a period to view its trial balance.
          </p>
          <Link to="/periods"
        ...
        <>
          {/* Header Section (Mata uang IDR tertera di sini) */}
          <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
              <h2 className="mb-1 fw-bold text-white d-flex align-items-center">
                <i className="ti ti-list me-2 text-info fs-2"></i> Trial Balance (Unadjusted)
              </h2>
              <p className="text-white-50 mb-0">
                Balances of active accounts prior to adjusting entries (in IDR).
              </p>
            </div>
            <div className="d-flex gap-2">
              <Link to="/reports/general-ledger/permanent"
                className="btn btn-outline-secondary fw-semibold shadow-sm d-inline-flex align-items-center"
              >
                <i className="ti ti-book me-1"></i> General Ledger
              </Link>
            </div>
          </div>

          {/* Trial Balance Table Card */}
          <div className="card bg-body-tertiary border-secondary text-white shadow-sm border border-secondary border-opacity-25 rounded-4">
            ...
            >
              {isBalanced ? (
                <>
                  ...
                  <span>Total Debit equals Total Credit. The trial balance is balanced.</span>
                </>
              ) : (
                <>
                  <i className="ti ti-alert-triangle-filled text-danger me-2 fs-5 flex-shrink-0"></i>
                  <span>The trial balance is unbalanced. Please review your journal entries.</span>
                </>
              )}
            </div>
          )}
