        ...
        <CardContent className="p-6 md:p-8">
          ...
          <div className="text-center mt-6">
            ...
            <div className="flex justify-center gap-3 mt-6 flex-wrap">
              <Button asChild className="rounded-xl bg-gradient-to-br from-indigo-500/80 to-violet-600/80 border border-indigo-300/20 shadow-lg hover:from-indigo-500 hover:to-violet-600">
                <Link to="/dashboard" className="flex items-center gap-2"><IconDashboard size={16}/> Dashboard</Link>
              </Button>
              <Button asChild variant="secondary" className="rounded-xl bg-white/10 text-white hover:bg-white/15 border border-white/10">
                <Link to="/journal-entry" className="flex items-center gap-2"><IconNotebook size={16}/> General Journal</Link>
              </Button>
            </div>
          </div>
        </CardContent>
    ...
    <div className="rounded-xl bg-[#0F172A]/60 border border-white/5 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {loading? <Skeleton className="h-4 w-4 rounded-full"/> : <Button variant="ghost" size="icon" className="h-6 w-6 text-white/60 hover:text-white" onClick={fetchMarketData}><IconRefresh size={14}/></Button>}
          <Badge variant="outline" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-">LIVE</Badge>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {loading? [1,2,3].map(i=><Skeleton key={i} className="h-20 rounded-lg bg-white/5"/>) :
          marketData.map((item,idx)=>(
          <div key={idx} className="rounded-lg border border-white/10 bg-black/20 p-2.5 flex flex-col justify-between min-h-">
            <div className="flex justify-between items-center"><span className="text-xs font-bold text-white">{item.symbol}</span><Badge className={`text- ${item.isUp?'bg-emerald-500/15 text-emerald-400':'bg-red-500/15 text-red-400'} border-0 flex items-center`}>{item.isUp? <IconTrendingUp size={10} className="mr-0.5"/>:<IconTrendingDown size={10} className="mr-0.5"/>}{item.change}</Badge></div>
