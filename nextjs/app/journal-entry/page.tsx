...
function JournalEntryContent() {
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      try {
        ...
        const accountsRes = await fetch(`${API_BASE_URL}/api/v1/api/v1/chart-of-accounts`, {
          method: 'GET',
          headers,
        });
        ...
        if (isEdit && entryIdParam) {
          const journalRes = await fetch(`${API_BASE_URL}/api/v1/api/v1/journals/${entryIdParam}`, {
            method: 'GET',
            headers,
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_BASE_URL}/api/v1/api/v1/journals/${entryIdParam}`
        : `${API_BASE_URL}/api/v1/api/v1/journals`;
