...
export default function AdjustingJournalPage() {
  ...
  const fetchJournalData = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/reports/adjusting-journal`, {
        method: 'GET',
  ...
  const deleteEntry = async (entry: JournalEntry) => {
    ...
    setErrorMessage(null);
    try {
      const response = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/reports/adjusting-journal/${entry.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
