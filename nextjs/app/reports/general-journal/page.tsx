...
export interface JournalLine { id: number; lineOrder: number; debit: number; credit: number; lineDescription?: string; accountId?: number; accountName?: string; referenceNumber?: number; account?: Account; }
...
export default function GeneralJournalPage() {
  ...
  const fetchJournalData = async () => {
    ...
    try {
      const res = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/reports/journals/general`, { method: 'GET', headers: { 'Content-Type': 'application/json' }, credentials: 'include' });
      if (res.status === 401) { handleUnauthorized(); return; }
      if (!res.ok) throw new Error('Failed to load general journal data.');
      const data = await res.json();
      if (data.success) { setSelectedPeriodName(data.selectedPeriodName || null); setIsPeriodClosed(data.isPeriodClosed || false); setEntries(Array.isArray(data.entries)? data.entries : []); }
      else throw new Error(data.message || 'Failed to parse journal data.');
    } catch (err: any) { setErrorMessage(err.message); setEntries([]); }
  ...
  const deleteEntry = async (entry: JournalEntry) => {
    ...
    if (!window.confirm(`Delete journal entry ${entry.transactionNumber}?`)) return;
    setErrorMessage(null);
    try {
      const res = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/reports/journals/general/${entry.id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, credentials: 'include' });
      if (res.status === 401) { handleUnauthorized(); return; }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 text-2xl font-bold tracking-tight"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10 text-amber-500"><IconBook className="h-5 w-5" /></span> General Journal</h2>
          <p className="text-sm text-muted-foreground">Chronological record of financial transactions {selectedPeriodName? `(Viewing Period: ${selectedPeriodName})` : ''}</p>
        </div>
        <div className="flex gap-2">
          <Button asChild><Link href="/journal-entry"><IconPlus className="h-4 w-4" /> Add Entry</Link></Button>
              ...
              <TableBody>
                {loading? (
                  ...
                  return sortedLines.map((line, i) => {
                    ...
                    return (
                      <TableRow key={`${entry.id}-${line.id || i}`} className={entryShade}>
                        <TableCell className="pl-6 align-top py-2">
                          ...
                          {isFirstRow && (
                            <div className="mt-1 flex flex-col items-start gap-1">
                              ...
                              {editMode && <div className="mt-1 flex gap-1"><Button asChild variant="ghost" size="icon" className="h-6 w-6"><Link href={`/journal-entry?id=${entry.id}`}><IconPencil className="h-3.5 w-3.5" /></Link></Button><Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={()=>deleteEntry(entry)}><IconTrash className="h-3.5 w-3.5" /></Button></div>}
                            </div>
                          )}
                        </TableCell>
                        ...
                        <TableCell className="text-center align-top py-2"><code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs font-bold text-amber-500">{refNum}</code></TableCell>
                  ...
                  <TableRow><TableCell colSpan={6} className="py-12 text-center">
                    {selectedPeriodName === null? <div className="flex flex-col items-center"><IconEyeOff className="mb-2 h-8 w-8 text-muted-foreground" /><h6 className="font-semibold">No Period Selected</h6><p className="text-xs text-muted-foreground">Go to <Link href="/periods" className="font-bold text-primary underline">Periods</Link> to select one.</p></div>
                    : <div className="flex flex-col items-center"><IconBookOff className="mb-2 h-8 w-8 text-muted-foreground" /><h6 className="font-semibold">No Entries Found</h6><p className="text-xs text-muted-foreground">No journal entries recorded in <strong>{selectedPeriodName}</strong>.</p></div>}
                  </TableCell></TableRow>
                )}
              </TableBody>
            </Table>
