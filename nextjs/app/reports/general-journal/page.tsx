...
export default function GeneralJournalPage() {
  ...
  // Helper untuk HTTP 401 Unauthorized
  ...
  const fetchJournalData = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/api/v1/reports/general-journal`, {
        method: 'GET',
      ...
      if (resData.success) {
        ...
        throw new Error(resData.message || 'Failed to parse journal data.');
      }
    } catch (error: any) {
  ...
  const deleteEntry = async (entry: JournalEntry) => {
    ...
    const confirmed = window.confirm(
      `Delete journal entry ${entry.transactionNumber}? This action cannot be undone.`
    );
    if (!confirmed) return;

    setErrorMessage(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/api/v1/reports/general-journal/${entry.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
  ...
  // Pelacakan pengelompokan tanggal per-render
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
          <h2 className="mb-1 fw-bold text-white d-flex align-items-center">
            <i className="ti ti-book me-2 text-warning fs-2"></i> General Journal
          </h2>
          <p className="text-white-50 mb-0">
            Chronological record of financial transactions {selectedPeriodName ? `(Viewing Period: ${selectedPeriodName})` : ''}.
          </p>
        </div>
        <div className="d-flex gap-2">
          <Link href="/journal-entry" className="btn btn-primary fw-semibold shadow-sm d-inline-flex align-items-center">
            <i className="ti ti-plus me-1"></i> Add Entry
          </Link>
          <button
            type="button"
            className={`btn btn-outline-secondary fw-semibold shadow-sm d-inline-flex align-items-center ${editMode ? 'active' : ''}`}
          ...
          <div className="table-responsive">
            <table className="table table-dark table-hover align-middle mb-0">
              <thead className="table-active border-bottom border-secondary text-secondary">
                <tr>
                  <th className="ps-4" style={{ width: '14%' }}>
                    Date &amp; Ref
                  </th>
                  ...
                  <th className="text-end pe-4" style={{ width: '12%' }}>
                    Credit
                  </th>
                </tr>
              </thead>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={6} className="text-center py-5 text-white-50">
                      ...
                      <span>Loading general journal from the server...</span>
                    </td>
                  </tr>
                ) : entries.length > 0 ? (
                  entries.map((entry) => {
                        ...
                        <tr key={`${entry.id}-${line.id || i}`} className={entryShade}>
                          <td className="ps-4 text-nowrap align-top py-2">
                            ...
                            {isFirstRow && (
                              <div className="d-flex flex-column align-items-start mt-1">
                                <small className="text-warning fw-semibold">{entry.transactionNumber}</small>
                                {entry.createdAt && (
                                  <span
                          ...
                          <td
                            className={`align-top py-2 ${
                              isDebit ? 'fw-semibold text-white' : 'ps-4 text-white-50'
                            }`}
                          >
                            {accountName}
                          </td>
                          <td className="small align-top py-2 text-white-50">
                          ...
                          <td className="text-end align-top py-2 text-success fw-semibold">
                            {line.debit > 0 ? formatNumber(line.debit) : '-'}
                          </td>
                          <td className="text-end pe-4 align-top py-2 text-danger fw-semibold">
                    ...
                    <td colSpan={6} className="text-center py-5 text-muted">
                      {selectedPeriodName === null ? (
                        <div className="py-3">
                          ...
                          ></i>
                          <h6 className="fw-semibold text-white">No Period Selected</h6>
                        ...
                        <div className="py-3">
                          <i
                            className="ti ti-book-off mb-2 d-block mx-auto text-secondary"
                            style={{ fontSize: '2.5rem' }}
                          ></i>
                          <h6 className="fw-semibold text-white">No Entries Found</h6>
                          <p className="mb-0 small text-white-50">
                            No journal entries recorded in{' '}
