...
export default function UnadjustedTrialBalancePage() {
  ...
  const fetchTrialBalanceData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${NEXT_PUBLIC_API_URL}/api/v1/reports/trial-balance/unadjusted`, { method: 'GET', headers: { 'Content-Type': 'application/json' }, credentials: 'include' });
      if (res.status === 404 || res.status === 400) { setNoPeriodSelected(true); setRows([]); return; }
      if (res.status === 401) { setErrorMessage('Session expired or unauthorized. Please login again.'); setRows([]); return; }
      if (!res.ok) throw new Error('Failed to load Unadjusted Trial Balance data.');
  ...
  if (loading) return <div className="mx-auto max-w-6xl p-6 flex justify-center gap-2 text-sm text-muted-foreground"><IconLoader2 className="h-4 w-4 animate-spin" /> Loading Trial Balance...</div>;
    ...
    <div className="mx-auto max-w-6xl space-y-4 p-4 md:p-6">
      ...
      {noPeriodSelected? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mb-4 max-w-sm text-sm text-muted-foreground">This report follows whichever period you're viewing. Select a period to view its trial balance.</p>
        ...
        <>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="flex items-center gap-2 text-2xl font-bold tracking-tight"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-500/10 text-sky-500"><IconList className="h-5 w-5" /></span> Trial Balance (Unadjusted)</h2>
              <p className="text-sm text-muted-foreground">Balances of active accounts prior to adjusting entries (in IDR).</p>
            </div>
            <Button asChild variant="outline"><Link href="/reports/general-ledger/permanent"><IconBook className="h-4 w-4" /> General Ledger</Link></Button>
          </div>
          ...
          {rows.length > 0 && (
            <Alert variant={isBalanced? 'default' : 'destructive'} className={isBalanced? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : ''}>
              ...
              <AlertDescription>{isBalanced? 'Total Debit equals Total Credit. The trial balance is balanced.' : 'The trial balance is unbalanced. Please review your journal entries.'}</AlertDescription>
            </Alert>
          )}
        </>
      )}
