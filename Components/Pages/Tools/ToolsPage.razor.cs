...
public partial class ToolsPage : ComponentBase
{
    ...
    protected async Task HandleRecalculateLedger()
    {
        bool confirmed = await JS.InvokeAsync<bool>("confirm", "This will rebuild all running ledger balances. Continue?");
        if (!confirmed) return;
        ...
        {
            errorMessage = $"Failed to recalculate ledger: {ex.Message}";
        }
        finally
        {
            isBusy = false;
    ...
    {
        Nav.NavigateTo("/api/Tools/BackupDatabase", true);
    }
}
